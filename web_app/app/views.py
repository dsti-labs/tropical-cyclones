...
def make_prediction(request):
    try:
        if request.method == "POST":
            ...
            cyclone = Cyclone.objects.filter(id=cyclone_id).update(stage=result)
            
            logging.debug(f"cyclone {cyclone_id} updated")

            return JsonResponse({"message": "Form submitted!", "data":cyclone, "status":200})
...
def display_most_recent_predictions(request, nb_predictions:int):
    try:
        ...
        if nb_predictions > 1:
            logging.debug(f"fetching the last {str(nb_predictions)} cyclones in the database")
        else:
        ...
        latest_cyclones = Cyclone.objects.filter(cyclone_id__in=recent_cyclone_ids).order_by("-timestamp")

        if latest_cyclones.exists():
            data = list(latest_cyclones.values())
            
            return JsonResponse({"message": "Form submitted!", "data":data, "status":200})
